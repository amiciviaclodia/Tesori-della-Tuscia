...
function buildRouteCluesFromOrder(profile, clueOrder) {
...
function ensureTeamRoute(team, profile) {
  ...
  team.routeClues = buildRouteCluesFromOrder(profile, team.clueOrder);

  return team.routeClues;
}

function getOrderedCluesForTeam(team, profile) {
  const routeClues = ensureTeamRoute(team, profile);
  return routeClues;
}
...
function getCurrentClueForTeam(team) {
  const profile = ensureProfile(team.profileId);
  const routeClues = getOrderedCluesForTeam(team, profile);
  return routeClues[team.clueIndex] || null;
}
...
function completeTeamIfNeeded(team) {
  const profile = ensureProfile(team.profileId);
  const routeClues = getOrderedCluesForTeam(team, profile);

  if (team.clueIndex >= routeClues.length) {
...
app.get('/api/bootstrap', (req, res) => {
  res.json({
    ...
    state: {
      game,
      teams: Object.values(teams).map(serializeTeam)
    }
  });
});
...
app.post('/api/teams', (req, res) => {
  try {
    ...
    res.json({ team: serializeTeam(teams[id]) });
  } catch (e) {
...
app.post('/api/teams/:id/verify-code', (req, res) => {
  try {
    const team = getTeam(req.params.id);
    const clue = getCurrentClueForTeam(team);
...
app.listen(PORT, () => {
  console.log("Server V4.2 attivo su porta", PORT);
});